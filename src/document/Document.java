...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < word.length(); i++) {
			if ( (Character.toLowerCase(word.charAt(i)) == 'a') || (Character.toLowerCase(word.charAt(i)) == 'e') || 
		         ...
		         (Character.toLowerCase(word.charAt(i)) == 'u') || (Character.toLowerCase(word.charAt(i)) == 'y') ) {
				if( !contiguous && !( (Character.toLowerCase(word.charAt(i)) == 'e') && (i == word.length()-1 ) && (syllableNum != 0) ) ) {
					contiguous = true;
					syllableNum ++;
				}
			}
	...
	public double getFleschScore()
	{
		...
		double fleschScore = 206.835 - 1.015 * (nword / nsentence) - 84.6 * (nsyllable / nword);
	    return fleschScore;
	}
